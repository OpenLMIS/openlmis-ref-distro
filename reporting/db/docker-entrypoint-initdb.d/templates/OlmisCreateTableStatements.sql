...
SELECT r.*, li.*, sh.*, rd.*, f.status as facility_status, fa.* 
FROM requisitions r 
LEFT JOIN requisitions_status_history sh ON r.id::VARCHAR = sh.requisition_id
...
CASE WHEN (SUM(stock_on_hand) = 0 OR SUM(total_stockout_days) > 0 
OR SUM(beginning_balance) = 0 OR SUM(max_periods_of_stock) = 0) THEN 1 ELSE 0 END as combined_stockout,
CASE
...
GROUP BY requisition_line_item_id, requisition_id) li ON r.id::VARCHAR = li.requisition_id WITH DATA;
